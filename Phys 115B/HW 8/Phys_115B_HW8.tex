...
\begin{proof}
    ...
    \begin{itemize}
        \item[(a)] Let $N_\text{tot}:= N_++N_-$, and $N_\text{com}:= \min\{N_+,N_-\}$ (so, how many electrons in total, and how many pairs of opposit spin electrons, respectively). Another way to represent the numbers is $N_\text{com} = N_\text{tot}$
        
        Since each energy level allows two electrons with opposite spins, it's the most favoarable for the lowest energy levels to be filled with 
