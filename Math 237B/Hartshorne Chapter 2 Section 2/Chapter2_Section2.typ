...
#theorem("Structure of Projective Space")[
  ...
  2. For any homogeneous element $f in S_+$, define $D_+(f) = X\\V((f))$, then $D_+(f)$ is an affine scheme, with $D_+(f) tilde.equiv Spec(S_((f)))$ (where $S_f$ is a $ZZ$-graded localization with $deg(s/f^n) := deg(s)-n$ for all homogeneous element $s in S$, and $S_((f)):= (S_f)_0$).
][
